    ...
    if message.chat.id == config.admin_id:
        try:
            if message.text == 'Отмена':
                bot.send_message(config.admin_id, "Вы отменили действие",
                ...
                bot.send_message(
                    config.admin_id, '✅ Рассылка была успешно отправлена!', reply_markup=menu.admin)
                for user in all_users:
                    bot.send_message(user, message.text,
                                     reply_markup=menu.admin)
        except:
            bot.send_message(
                config.admin_id, "❌ Ошибка! Отправлять можно только текст.", reply_markup=menu.admin)
    ...
    if message.chat.id == config.admin_id:
        try:
            if message.text == 'Отмена':
                ...
                bot.send_message(
                    config.admin_id, '✅ Сохранено!\n\nДля корректной работы необходимо перезапустить бота.', reply_markup=menu.new_answer)
        except:
            bot.send_message(
                config.admin_id, "❌ Ошибка! Отправлять можно только текст.", reply_markup=menu.new_answer)
...
def qiwi_edit_number(message):
    if message.chat.id == config.admin_id:
        try:
            ...
            bot.send_message(
                config.admin_id, "❌ Ошибка! Отправлять можно только текст.", reply_markup=menu.payments)
...
def send_welcome(message):
    if not str(message.chat.id) in all_users:
        ...
        bot.send_message(message.chat.id, "{}\n\n{}".format(
            config.name, config.hello_text), reply_markup=menu.operator)
...
def send_otziv(message):
    if message.text == '📦 Покупки':
        if message.chat.id == config.admin_id:
            logging.info("The admin clicked purchases. ID: " +
                         str(message.chat.id))
            bot.send_message(message.chat.id, '❌ Покупки отсутствуют ❌')
        else:
            logging.info("The user clicked purchases. ID: " +
                         str(message.chat.id))
            bot.send_message(message.chat.id, '❌ Покупки отсутствуют ❌')
        ...
        if message.chat.id == config.admin_id:
            bot.send_message(
                config.admin_id, "Здесь Вы можете настроить и обновить Ваши платёжные данные.", reply_markup=menu.payments)
    elif message.text == 'Настройка Qiwi':
        if message.chat.id == config.admin_id:
            bot.send_message(config.admin_id, "Настройка Qiwi кошелька",
                             reply_markup=menu.qiwi_edit)
    elif message.text == 'Настройка Bitcoin':
        if message.chat.id == config.admin_id:
            msg = bot.send_message(
                config.admin_id, "Введите новый адрес bitcoin-кошелька: ", reply_markup=menu.otmena)
        ...
        if message.chat.id == config.admin_id:
            ...
            bot.send_message(config.admin_id, '<a><b>{}\n\n🥝 QIWI:\nНомер:</b> {}\n<b>Токен:</b> {}\n\n<b>💰BITCOIN:\nАдрес:</b> {}</a>'.format(
                text_1, text_2, text_3, text_4), parse_mode='HTML')
    elif message.text == '📲 Настройка ответов':
        if message.chat.id == config.admin_id:
            bot.send_message(
                config.admin_id, "В этом разделе вы можете полностью настроить и отредактировать информационный отдел.", reply_markup=menu.new_answer)
    elif message.text == '📩 Рассылка':
        if message.chat.id == config.admin_id:
            msg = bot.send_message(
                config.admin_id, "Для отправки сообщения необходимо ввести текст:", reply_markup=menu.otmena)
        ...
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Moscow. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_msk)
    elif message.text == 'Санкт-Петербург':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Saint Petersburg. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_spb)
    elif message.text == 'Екатеринбург':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Yekaterinburg. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_ekb)
    elif message.text == 'Домодедово':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Domodedovo. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_msk)
    elif message.text == 'Мытищи':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Mytishchi. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_spb)
    elif message.text == 'Зеленоград':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Zelenograd. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_perm)
    elif message.text == 'Подольск':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Podolsk. ID: "+str(message.chat.id))
        bot.send_message(message.chat.id, '⬇️ Какой район интересует? ⬇️',
                         reply_markup=menu.rayon_voronezh)
    elif message.text == 'Челябинск':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Chelyabinsk. ID: "+str(message.chat.id))
        bot.send_message(message.chat.id, '⬇️ Какой район интересует? ⬇️',
                         reply_markup=menu.rayon_voronezh)
    elif message.text == 'Нижний Новгород':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Nizhny Novgorod. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_spb)
    elif message.text == 'Иркутск':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Irkutsk. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_msk)
    elif message.text == 'Оренбург':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Orenburg. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_perm)
    elif message.text == 'Пенза':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Penza. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_spb)
    elif message.text == 'Омск':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Omsk. ID: "+str(message.chat.id))
        bot.send_message(message.chat.id, '⬇️ Какой район интересует? ⬇️',
                         reply_markup=menu.rayon_voronezh)
    elif message.text == 'Пермь':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Perm. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_perm)
    elif message.text == 'Рязань':
        logging.info(str(message.from_user.first_name) +
                     " chose the city of Ryazan. ID: "+str(message.chat.id))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_msk)
    elif message.text in menu.rayon_spisok:
        logging.info(str(message.from_user.first_name) + ' chose the area ' +
                     message.text + '. ID: ' + str(message.chat.id))
        ...
        try:
            ...
            while True:
                if api.check(comment):
                    ...
                    print("New payment!")
                    bot.send_message(config.admin_id, "<a><b>🎉 Пополнение!\n\nПользователь:</b> {}\n<b>Дата:</b> {}\n<b>Платёжка:</b> QIWI</a>".format(
                        message.chat.id, date), parse_mode='HTML')
